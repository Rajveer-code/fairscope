...
The most closely related recent work is the 2025 toolbox \emph{meval}~\cite{meval}, a
statistical toolbox for stratified, fine-grained model-performance analysis that
\emph{also} provides subgroup metric uncertainty and multiple-comparison corrections (with a
medical-imaging focus). \texttt{fairscope} overlaps with meval on uncertainty and
significance; what it adds is the specific DeLong per-subgroup AUC intervals, the per-subgroup
calibration \emph{and recalibration} interface, the five-axis cross-platform CPFE protocol,
and one-call domain audits (healthcare, lending, federated), packaged as a single tested
library. Relative to AIF360 and Fairlearn, the subgroup DeLong interval, per-subgroup ECE, and
corrected significance test are not first-class functions at all (Section~\ref{sec:compare}).

\emph{Verification note: the capability comparison in Section~\ref{sec:compare} was checked
against AIF360~0.6.1 and Fairlearn~0.14.0 (June 2026) by inspecting their installed public
APIs, and meval against its arXiv description; re-confirm if those versions change before
submission.}
...
stability); \texttt{lending/} (a descriptive annual approval-gap analysis plus an optional
subgroup conditional-average-treatment-effect audit via Causal Forest double machine
learning, behind an \texttt{econml} extra); and \texttt{federated/} (a cross-node audit:
per-node DeLong intervals, cross-node disparity testing with multiple-comparison control, and
per-node recalibration). The \texttt{lending/} and \texttt{federated/} modules are
composition over \texttt{core/} and ports of documented methods---only CPFE is novel.
...
Subgroup significance test $+$ correction & no & no & yes \\
Subgroup-stratified recalibration (temp./isotonic) & partial$^{\dagger}$ & no & yes \\
...
\bottomrule
\end{tabular}
\caption{Capability comparison, verified against AIF360~0.6.1 and Fairlearn~0.14.0
(June 2026). $^{*}$Fairlearn's \texttt{MetricFrame} computes per-subgroup AUC point estimates
but no analytic (DeLong) interval. $^{\dagger}$AIF360 provides
\texttt{CalibratedEqOddsPostprocessing} (calibrated equalized-odds postprocessing), not a
general per-subgroup recalibration interface.}
\end{table}
...
\begin{itemize}
  ...
  \item Gradient-saliency attribution is a local linear approximation and can be unstable
    ...
    definitive.
  \item The capability comparison (Section~\ref{sec:compare}) reflects AIF360~0.6.1 and
    Fairlearn~0.14.0 as of June 2026; toolkit APIs evolve, so it should be re-checked against
    the releases current at publication.
\end{itemize}
